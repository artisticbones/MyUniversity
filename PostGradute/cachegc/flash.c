			...
			case 0:
			{
				...
				if((sub_req->cacheFlag == 0) && ssd->dram->map->map_entry[sub_req->lpn].pn == 0){
					...
					if(validPageCacheNode && (validPageCacheNode->lpn == sub_req->lpn)){
						...
						validPageCacheNode->vaildState |= sub_req->state;
						if(update){
							free(update);
							sub_req->update = NULL;
						}
	...
	{	
		...
		if (flag==0)
		{
			if(ssd->dram->map->map_entry[sub->lpn].pn == 0){
				...
				if(validPageCacheNode && (validPageCacheNode->lpn == sub->lpn)){
...
struct ssd_info *process(struct ssd_info *ssd)   
{
	...
	**********************************************************/
	...
	{
		struct ValidPageCacheNode* validPageCacheNode = ssd->validPageCache.head;
		//validPageCacheNode = NULL;
		if(validPageCacheNode){
			unsigned char loopFlag = 0;
			unsigned char validPageCacheNodeType;
			if(!validPageCacheNode->subFlag){
				creat_sub_request(ssd, validPageCacheNode->lpn,size(validPageCacheNode->vaildState), validPageCacheNode->vaildState, ssd->validPageCacheReq, WRITE, TARGET_LSB);
				validPageCacheNode->subFlag = 1;
				loopFlag = 1;
				validPageCacheNodeType = validPageCacheNode->type;
			}
			validPageCacheNode = validPageCacheNode->next;
			while(validPageCacheNode){
				if(validPageCacheNode->type == validPageCacheNodeType || !loopFlag)
					if(!validPageCacheNode->subFlag){
						creat_sub_request(ssd, validPageCacheNode->lpn,size(validPageCacheNode->vaildState), validPageCacheNode->vaildState, ssd->validPageCacheReq, WRITE, TARGET_LSB);
						validPageCacheNode->subFlag = 1;
						loopFlag = 1;
