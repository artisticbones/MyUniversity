...
Status move_page_cache(struct ssd_info * ssd, struct local *location, unsigned int * transfer_size)
{
	unsigned int free_state=0,valid_state=0;
	unsigned int lpn=0,old_ppn=0;
	...
	struct ValidPageCacheNode* newNode = malloc(sizeof(struct ValidPageCacheNode));
	alloc_assert(newNode,"ValidPageCacheNode");
	
	newNode->lpn = lpn;
	newNode->freeState = free_state;
	newNode->vaildState = valid_state;
	newNode->type = ssd->validPageCache.nowType;
	newNode->next = ssd->validPageCache.head;
	newNode->subFlag = 0;
	//printf("lpn %u\n", newNode->lpn);
	ssd->validPageCache.head = newNode;
	...
	if (old_ppn==ssd->dram->map->map_entry[lpn].pn)                                                     /*修改映射表*/
	{
		...
		printf(" cache old != %ld lpn is %d ssd->dram->map->map_entry[lpn].pn is %d old is %d\n", ssd->completed_request_count, lpn, ssd->dram->map->map_entry[lpn].pn, old_ppn);
		while(1);
	}
...
int uninterrupt_gc(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane,struct gc_operation *gc_node)       
{
	...
	unsigned int page_move_count_cache = 0, transfer_size_cache = 0;
	...
	invalid_page=0;
	page_move_count_cache = 0;
	transfer_size_cache = 0;
	transfer_size=0;
	block=-1;
	...
	{		
		...
		{	
			...
			location->page=i;
			if(ssd->validPageCache.freePage > 0){
				//printf("123 lpn %u", ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].page_head[i].lpn);
				if(move_page_cache(ssd, location, &transfer_size_cache) == SUCCESS){
					ssd->validPageCacheCount++;
					page_move_count_cache++;
				}	
				else {
					//ssd->page_move_count++;
	...
	if(page_move_count_cache != 0)
		ssd->validPageCache.nowType++;
	...
	***************************************************************/
	...
	else
	{
		...
		ssd->channel_head[channel].next_state_predict_time += page_move_count_cache*(7*ssd->parameter->time_characteristics.tWC+ssd->parameter->time_characteristics.tR)+transfer_size_cache*SECTOR*(ssd->parameter->time_characteristics.tRC);
		ssd->channel_head[channel].chip_head[chip].next_state_predict_time=ssd->channel_head[channel].next_state_predict_time+ssd->parameter->time_characteristics.tBERS;
